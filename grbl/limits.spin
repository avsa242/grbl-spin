{{
  limits.c - code pertaining to limit-casees and performing the homing cycle
  Part of Grbl
...
#define HOMING_AXIS_SEARCH_SCALAR  1.5 ' Must be > 1 to ensure limit case will be engaged.
#endif
#ifndef HOMING_AXIS_LOCATE_SCALAR
#define HOMING_AXIS_LOCATE_SCALAR  5.0 ' Must be > 1 to ensure limit case is cleared.
#endif
...
' limit case can cause a lot of problems, like false readings and multiple interrupt calls.
' If a case is triggered at all, something bad has happened and treat it as such, regardless
' if a limit case is being disengaged. It's impossible to reliably tell the state of a
...
' the trigger point of the limit casees. The rapid stops are handled by a system level axis lock
...
'    long dual_trigger_position
    ...
    ' default, Grbl defines machine space as all negative, as do most CNCs. Since limit casees
    ' can be on either side of an axes, check and set axes machine zero appropriately. Also,
    ' set up pull-off maneuver from axes limit casees that have been homed. This provides
    ' some initial clearance off the casees and should also help prevent them from falsely
    ' triggering when hard limits are enabled or when more than one axes shares a limit pin.
    ' Set machine positions for homed limit casees. Don't update non-homed axes.
    repeat idx from 0 to N_AXIS-1
        ' NOTE: settings.max_travel[] is stored as a negative value.
...
PUB limits_soft_check({float *}target)
