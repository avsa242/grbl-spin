...
PUB{uint8_t} gc_execute_line({char *}line) | {uint8_t}axis_command, axis_0, axis_1, axis_linear, coord_select, axis_words, ijk_words, {uint16_t}command_words, value_words, {uint8_t}gc_parser_flags, {uint8_t}word_bit, char_counter, {char}letter, {float}value, {uint8_t}int_value, {uint16_t}mantissa, {uint8_t}idx, {float}block_coord_system[N_AXIS], x, y, h_x2_div_d, target_r, delta_r, {plan_line_data_t} plan_data, {*}pl_data, status, {uint8_t}gc_update_pos
  ...
  {{ -------------------------------------------------------------------------------------
     ...
     a number, which can either be a  command or sets/assigns a command value. Also,
     perform initial error-checks for command word modal group violations, for any repeated
     words, and for negative values set for the value words F, N, P, T, and S. }}

  'byte word_bit ' Bit-value for assigning tracking variables
    int_value := 0
    mantissa := 0
    ...
    repeat while (line[char_counter] <> 0) ' Loop until no more g-code words in line.
        ...
        case(letter)
            ...
            "G":
                ' Determine  command and its modal group
                case(int_value) 
                    10, 28, 30, 92:'10: case 28: case 30: case 92:
                    ...
                    ' Ignore G28.1, G30.1, and G92.1
                            ...
                            ' No break. Continues to next line.     'XXX Not sure what the intent was here...in the original C source, the next case (or any of the remaining ones) could never have matched if this one matched, I don't think? How could int_value have changed within these few lines?
                    4, 53:
                        word_bit := MODAL_GROUP_G0
                        gc_block.non_modal_command := int_value
                        if ((int_value == 28) OR (int_value == 30) OR (int_value == 92)) 
                            if (NOT ((mantissa == 0) OR (mantissa == 10)) )
                    ...
                    0, 1, 2, 3, 38:
                        ...
                        ' No break . Continues to next line.    'XXX same as above?
                    80:
                        word_bit := MODAL_GROUP_G1
                        gc_block.modal.motion := int_value
                        if (int_value == 38)
                            if (NOT((mantissa == 20) OR (mantissa == 30) OR (mantissa == 40) OR (mantissa == 50)))
                    ...
                    90, 91:
                        if (mantissa == 0)
                            ...
                            if ((mantissa <> 10) OR (int_value == 90)) 
                    ...
                    OTHER: FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND) ' [Unsupported G command]
                if (mantissa > 0)
                    FAIL(STATUS_GCODE_COMMAND_VALUE_NOT_INTEGER)  ' [Unsupported or invalid Gxx.x command]
                ' Check for more than one command per modal group violations in the current block
                ' NOTE: Variable  is always assigned, if the command is valid.
            ...
            "M":
                ...
                case(int_value) 
                    0, 1, 2, 30:
                        ...
                        case(int_value) 
                            0: gc_block.modal.program_flow := PROGRAM_FLOW_PAUSED ' Program pause
                            1:  ' Optional stop not supported. Ignore.
                            OTHER: gc_block.modal.program_flow := int_value ' Program end and reset
                    3, 4, 5:
                        word_bit := MODAL_GROUP_M7
                        case(int_value) 
                            3: gc_block.modal.spindle := SPINDLE_ENABLE_CW
                            4: gc_block.modal.spindle := SPINDLE_ENABLE_CCW
                            5: gc_block.modal.spindle := SPINDLE_DISABLE
#ifdef ENABLE_M7
                    7, 8, 9:
#else
                    8, 9:
                        ...
                        case(int_value)
#ifdef ENABLE_M7
                            7: gc_block.modal.coolant |= COOLANT_MIST_ENABLE
#endif
                            8: gc_block.modal.coolant |= COOLANT_FLOOD_ENABLE
                            9: gc_block.modal.coolant := COOLANT_DISABLE ' M9 disables both M7 and M8.
#ifdef ENABLE_PARKING_OVERRIDE_CONTROL
                    56:
                        word_bit := MODAL_GROUP_M9
                        gc_block.modal.override := OVERRIDE_PARKING_MOTION
#endif
                    OTHER: FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND) ' [Unsupported M command]
            ...
            {{ Non-Command Words: This initial parsing phase only checks for repeats of the remaining
               ...
               words (I,J,K,L,P,R) have multiple connotations and/or depend on the issued commands. }}
                ...
                ' NOTE: Variable  is always assigned, if the non-command letter is valid.
  ...
  {{ NOTE: At this point, the g-code block has been parsed and the block line can be freed.
     NOTE: It's also possible, at some future point, to  up STEP 2, to allow piece-wise
     ...
     new g-code block when the EOL character is received. However, this would  Grbl's startup
  ...
  '   is not defined after caseing to G94 from G93.
  ' NOTE: For jogging, ignore prior feed rate mode. Enforce G94 and check for required F word.
        ...
        if (gc_block.modal.units == UNITS_MODE_INCHES)
            ...
            if (gc_block.modal.feed_rate == FEED_RATE_MODE_INVERSE_TIME) ':= G93
                ...
                if (axis_command == AXIS_COMMAND_MOTION_MODE)
                    if ((gc_block.modal.motion <> MOTION_MODE_NONE) AND (gc_block.modal.motion <> MOTION_MODE_SEEK))
                        if (bit_isfalse(value_words,bit(WORD_F)))
                            FAIL(STATUS_GCODE_UNDEFINED_FEED_RATE)  ' [F word missing]
      
            ' NOTE: It seems redundant to check for an F word to be passed after caseing from G94 to G93. We would
            ...
            ' also allow other commands, following this case, to execute and not error out needlessly. This code is
            ...
            ' NOTE: If in G93 mode or caseed into it from G94, just keep F value as initialized zero or passed F word
    ...
    case (gc_block.modal.plane_select) 
        PLANE_SELECT_XY:
            axis_0 := X_AXIS
            axis_1 := Y_AXIS
            axis_linear := Z_AXIS
    ...
    if (gc_block.modal.units== UNITS_MODE_INCHES)
        repeat idx from 0 to N_AXIS-1' Axes indices are consistent, so loop may be used.
            if (bit_istrue(axis_words, bit(idx)) )
                gc_block.values.xyz[idx] *= MM_PER_INCH

    ' [13. Cutter radius compensation ]: G41/42 NOT SUPPORTED. Error, if enabled repeat while G53 is active.
    ...
    bytemove(block_coord_system, gc_state.coord_system, {sizeof}gc_state.coord_system)
    if ( bit_istrue(command_words, bit(MODAL_GROUP_G12)) )' Check if called in block
        ...
        if (gc_state.modal.coord_select <> gc_block.modal.coord_select) 
            if (NOT(settings_read_coord_data(gc_block.modal.coord_select, block_coord_system))) 
    ...
    case (gc_block.non_modal_command)
        ...
        ' NOTE: Tool offsets may be appended to these conversions when/if this feature is added.
            if (axis_command <> AXIS_COMMAND_TOOL_LENGTH_OFFSET ) ' TLO block any axis command.
                if (axis_words)
                    repeat idx from 0 to N_AXIS-1 ' Axes indices are consistent, so loop may be used to save flash space.
            ...
            case (gc_block.non_modal_command)
                NON_MODAL_GO_HOME_0, NON_MODAL_GO_HOME_1: ' G28, G30
                    ' [G28/30 Errors]: Cutter compensation is enabled.
                    ' Retreive G28/30 go-home position data (in machine coordinates) from EEPROM
                    ' NOTE: Store parameter data in IJK values. By rule, they are not in use with this command.
                    if (gc_block.non_modal_command == NON_MODAL_GO_HOME_0) 
                        if (NOT settings_read_coord_data(SETTING_INDEX_G28, gc_block.values.ijk)) 
                            FAIL(STATUS_SETTING_READ_FAIL)
                        else '== NON_MODAL_GO_HOME_1
                            if (NOT settings_read_coord_data(SETTING_INDEX_G30, gc_block.values.ijk)) 
                                FAIL(STATUS_SETTING_READ_FAIL) 
                    if (axis_words)
                    ' Move only the axes specified in secondary move.
                        repeat idx from 0 to N_AXIS-1 
                            if (NOT (axis_words & (1<<idx)) )
                ...
                NON_MODAL_ABSOLUTE_OVERRIDE:
                    ...
                    if (NOT (gc_block.modal.motion == MOTION_MODE_SEEK OR gc_block.modal.motion == MOTION_MODE_LINEAR))
                        FAIL(STATUS_GCODE_G53_INVALID_MOTION_MODE) ' [G53 G0/1 not active]
    ' [20. Motion modes ]:
    if (gc_block.modal.motion == MOTION_MODE_NONE) 
        ' [G80 Errors]: Axis word are programmed repeat while G80 is active.
    ...
    elseif (axis_command == AXIS_COMMAND_MOTION_MODE)
            ...
            case (gc_block.modal.motion) 
                MOTION_MODE_LINEAR:
                    ...
                    if (NOT axis_words) 
                        axis_command := AXIS_COMMAND_NONE 
                MOTION_MODE_CW_ARC: 
                    gc_parser_flags |= GC_PARSER_ARC_IS_CLOCKWISE ' No  intentional.
                MOTION_MODE_CCW_ARC:
                    ...
                    if (NOT axis_words)
                        FAIL(STATUS_GCODE_NO_AXIS_WORDS)  ' [No axis words]
                    if (NOT (axis_words & (bit(axis_0) | bit(axis_1)) )) 
                    ...
                    ' travel and thus we get the unadvisably long arcs as prescribed.
                        ...
                        if (NOT(ijk_words & (bit(axis_0) | bit(axis_1)) )) 
                ...
                MOTION_MODE_PROBE_TOWARD_NO_ERROR, MOTION_MODE_PROBE_AWAY_NO_ERROR:
                    gc_parser_flags |= GC_PARSER_PROBE_IS_NO_ERROR ' No  intentional.
                MOTION_MODE_PROBE_TOWARD, MOTION_MODE_PROBE_AWAY:
                    if ((gc_block.modal.motion == MOTION_MODE_PROBE_AWAY) OR (gc_block.modal.motion == MOTION_MODE_PROBE_AWAY_NO_ERROR))
    ...
    bytefill(pl_data, 0, {sizeof}plan_line_data_t) ' Zero pl_data struct
    ...
    if (gc_parser_flags & GC_PARSER_JOG_MOTION) 
        ...
        if (NOT(gc_block.non_modal_command == NON_MODAL_ABSOLUTE_OVERRIDE OR gc_block.non_modal_command == NON_MODAL_NO_ACTION)) 
            FAIL(STATUS_INVALID_JOG_COMMAND) 
    ...
    if (bit_istrue(settings.flags, BITFLAG_LASER_MODE)) 
        if ( NOT((gc_block.modal.motion == MOTION_MODE_LINEAR) OR (gc_block.modal.motion == MOTION_MODE_CW_ARC) OR (gc_block.modal.motion == MOTION_MODE_CCW_ARC)) )
            gc_parser_flags |= GC_PARSER_LASER_DISABLE

        ' Any motion mode with axis words is allowed to be passed from a spindle speed update. 
        ' NOTE: G1 and G0 without axis words sets axis_command to none. G28/30 are intentionally omitted.
        ' TODO: Check sync conditions for M3 enabled motions that don't enter the planner. (zero length).
        if (axis_words AND (axis_command == AXIS_COMMAND_MOTION_MODE))
            gc_parser_flags |= GC_PARSER_LASER_ISMOTION 
        else 
            ' M3 constant power laser requires planner syncs to update the laser when changing between
            ' a G1/2/3 motion mode state and vice versa when there is no motion in the line.
            if (gc_state.modal.spindle == SPINDLE_ENABLE_CW) 
                if ((gc_state.modal.motion == MOTION_MODE_LINEAR) OR (gc_state.modal.motion == MOTION_MODE_CW_ARC) OR (gc_state.modal.motion == MOTION_MODE_CCW_ARC))
    ...
    ' [4. Set spindle speed ]:
    if ((gc_state.spindle_speed <> gc_block.values.s) OR bit_istrue(gc_parser_flags, GC_PARSER_LASER_FORCE_SYNC))
   ...
   ' else pl_data->spindle_speed:= 0.0  ' Initialized as zero already.
    ...
    if (gc_state.modal.coolant <> gc_block.modal.coolant)
        ...
        ' can exist at the same time, repeat while coolant disable clears all states.
    ...
    if (gc_state.modal.coord_select <> gc_block.modal.coord_select)
        ...
        bytemove(gc_state.coord_system, block_coord_system, N_AXIS*{sizeof(float) <--- long?})
    system_flag_wco_change
    ...
    case(gc_block.non_modal_command)
        ...
        NON_MODAL_GO_HOME_0: case NON_MODAL_GO_HOME_1:
            ...
            bytemove(gc_state.position, gc_block.values.ijk, N_AXIS*{sizeof(float)})
        NON_MODAL_SET_HOME_0:
            settings_write_coord_data(SETTING_INDEX_G28,gc_state.position)
        NON_MODAL_SET_HOME_1:
            settings_write_coord_data(SETTING_INDEX_G30,gc_state.position)
        NON_MODAL_SET_COORDINATE_OFFSET:
            bytemove(gc_state.coord_offset, gc_block.values.xyz, {sizeof(gc_block.values.xyz)})
            system_flag_wco_change
        NON_MODAL_RESET_COORDINATE_OFFSET:
    ...
    if (gc_state.modal.motion <> MOTION_MODE_NONE)
        if (axis_command == AXIS_COMMAND_MOTION_MODE)
            gc_update_pos := GC_UPDATE_POS_TARGET
            if (gc_state.modal.motion == MOTION_MODE_LINEAR) 
                mc_line(gc_block.values.xyz, pl_data)
            elseif (gc_state.modal.motion == MOTION_MODE_SEEK) 
                pl_data->condition |= PL_COND_FLAG_RAPID_MOTION ' Set rapid motion condition flag.
                mc_line(gc_block.values.xyz, pl_data)
            elseif ((gc_state.modal.motion == MOTION_MODE_CW_ARC) OR (gc_state.modal.motion == MOTION_MODE_CCW_ARC))
                mc_arc(gc_block.values.xyz, pl_data, gc_state.position, gc_block.values.ijk, gc_block.values.r, axis_0, axis_1, axis_linear, bit_istrue(gc_parser_flags,GC_PARSER_ARC_IS_CLOCKWISE))
            else
        ...
        if (gc_update_pos == GC_UPDATE_POS_TARGET) 
            bytemove(gc_state.position, gc_block.values.xyz, {sizeof(gc_block.values.xyz)}) ' gc_state.position[]:= gc_block.values.xyz[]
        elseif (gc_update_pos == GC_UPDATE_POS_SYSTEM)
    ...
    if (gc_state.modal.program_flow) 
        ...
        if (gc_state.modal.program_flow == PROGRAM_FLOW_PAUSED) 
            ...
            if (sys.state <> STATE_CHECK_MODE) 
                if (NOT(settings_read_coord_data(gc_state.modal.coord_select, gc_state.coord_system))) 
                    FAIL(STATUS_SETTING_READ_FAIL) 
                system_flag_wco_change ' Set to refresh immediately just in something altered.
...
{{
  ...
  - Switches
   ...
   group 0:= 
    G92.2, G92.3 (Non modal: Cancel and re-enable G92 offsets)
   group 1:= 
    G81 - G89 (Motion modes: Canned cycles)
   group 4:= 
    M1 (Optional stop, ignored)
   group 6:= 
    M6 (Tool change)
   group 7:= 
    G41, G42 cutter radius compensation (G40 is supported)
   group 8:= 
    G43 tool length offset (G43.1/G49 are supported)
   group 8:= 
    M7* enable mist coolant (* Compile-option)
   group 9:= 
    M48, M49, M56* enable/disable override casees (* Compile-option)
   group 10:= 
    G98, G99 return mode canned cycles
   group 13:= 
