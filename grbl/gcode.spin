...
PUB gc_execute_line({char *}line) | {byte}axis_command, axis_0, axis_1, axis_linear, coord_select, axis_words, ijk_words, {uint16_t}command_words, value_words, {byte}gc_parser_flags, {uint8_t}word_bit, char_counter, {char}letter, {float}value, {uint8_t}int_value, {uint16_t}mantissa, {uint8_t}idx, {float}block_coord_system[N_AXIS], x, y, h_x2_div_d, target_r, delta_r, {plan_line_data_t} plan_data, {*}pl_data, status, {uint8_t}gc_update_pos
  {{ -------------------------------------------------------------------------------------
  ...
  'byte word_bit ' Bit-value for assigning tracking variables
    ...
    repeat while (line[char_counter] <> 0) 
    ...
    if (gc_state.modal.motion <> MOTION_MODE_NONE)
        if (axis_command == AXIS_COMMAND_MOTION_MODE)
            ...
            elseif (gc_state.modal.motion == MOTION_MODE_SEEK) 
                pl_data->condition |= PL_COND_FLAG_RAPID_MOTION ' Set rapid motion condition flag.
                mc_line(gc_block.values.xyz, pl_data)
            elseif ((gc_state.modal.motion== MOTION_MODE_CW_ARC) || (gc_state.modal.motion == MOTION_MODE_CCW_ARC)) 
