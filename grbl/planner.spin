...
PUB{static uint8_t } plan_prev_block_index({uint8_t} block_index)
...
PUB{static void} planner_recalculate | {uint8_t}block_index, {float}curr_entry_speed_sqr, {plan_block_t *} next_block[sizeof_plan_block_t/4], current[sizeof_plan_block_t/4]
    ...
    next_block := @block_buffer[block_buffer_planned] ' Begin at buffer planned pointer
...
PUB plan_discard_current_block | {uint8_t}block_index
...
PUB{plan_block_t *} plan_get_system_motion_block

    return(@block_buffer[block_buffer_head])

' Returns address of first planner block, if available. Called by various main program functions.
PUB{plan_block_t *} plan_get_current_block

    if (block_buffer_head == block_buffer_tail) 
        return(NULL)  ' Buffer empty
    return(@block_buffer[block_buffer_tail])

PUB {float} plan_get_exec_block_exit_speed_sqr | {uint8_t}block_index
...
PUB{uint8_t} plan_check_full_buffer
...
#else
        bytemove(position_steps, sys_position, {sizeof}sys_position) 
#endif
    else
        bytemove(position_steps, pl.position, {sizeof}pl.position)
#ifdef COREXY
...
PUB plan_sync_position | {uint8_t}idx
...
PUB{uint8_t} plan_get_block_buffer_available
...
PUB{uint8_t} plan_get_block_buffer_count
