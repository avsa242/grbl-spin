...
PUB plan_next_block_index({byte} block_index)
...
PUB{static byte } plan_prev_block_index({uint8_t} block_index)
...
{{                            PLANNER SPEED DEFINITION
  ...
  - block_buffer_planned: Points to the first buffer block after the last optimally planned block for normal
      streaming operating conditions. Use for planning optimizations by aing recomputing parts of the
...
PUB{static} planner_recalculate | {byte}block_index, {float}entry_speed_sqr
    ...
    ' Check if the first block is the tail. If so, notify stepper to update its current parameters.
     ...
     ' Three or more plan-able blocks
            repeat while (block_index <> block_buffer_planned) 
                next := current
...
PUB plan_reset
    ...
    bytefill(@pl, 0, {sizeof}planner_t) ' Clear planner struct
    plan_reset_buffer

PUB plan_reset_buffer
...
PUB plan_discard_current_block | {byte}block_index

    if (block_buffer_head <> block_buffer_tail) 
    ' Discard non-empty buffer.
        block_index:= plan_next_block_index( block_buffer_tail )
...
PUB plan_get_system_motion_block

  return(@block_buffer[block_buffer_head])

' Returns address of first planner block, if available. Called by various main program functions.
PUB plan_get_current_block

    if (block_buffer_head== block_buffer_tail) 
        return(NULL)  ' Buffer empty
    return(@block_buffer[block_buffer_tail])

PUB {float} plan_get_exec_block_exit_speed_sqr | {byte}block_index
...
PUB plan_update_velocity_profile_parameters | {byte} block_index, {plan_block_t *}block, {float}nominal_speed, {float}prev_nominal_speed
...
{{ Add a new linear movement to the buffer. target[N_AXIS] is the signed, absolute target position
   ...
   head. It as changing the planner state and preserves the buffer to ensure subsequent gcode
   motions are still planned correctly, repeat while the stepper module only points to the block buffer head
   to execute the special system motion. }}

PUB plan_buffer_line({float *}target, {plan_line_data_t *}pl_data) | {plan_block_t *}block, {long}target_steps[N_AXIS], {long}position_steps[N_AXIS], {float}unit_vec[N_AXIS], delta_mm, {byte}idx, {float}, {float}junction_unit_vec[N_AXIS], junction_cos_theta, junction_acceleration, sin_theta_d2, nominal_speed
  ...
  ' TODO: Need to check this method handling zero junction speeds when starting from rest.
    ...
    if (!(block->condition & PL_COND_FLAG_SYSTEM_MOTION)) 
        ...
        bytemove(pl.previous_unit_vec, unit_vec, {sizeof}unit_vec) ' pl.previous_unit_vec[]:= unit_vec[]
        bytemove(pl.position, target_steps, {sizeof}target_steps) ' pl.position[]:= target_steps[]
...
PUB plan_sync_position | {byte}idx
...
PUB plan_get_block_buffer_available
...
PUB plan_get_block_buffer_count
