...
' buffer. Normally, this buffer is partially in-use, but, for the worst scenario, it will
...
VAR
    ...
    long st_block_t 'typedef struct
    long {uint32_t} steps[N_AXIS]
    long {uint32_t} step_event_count
    byte {uint8_t} direction_bits
#ifdef ENABLE_DUAL_AXIS
    byte {uint8_t} direction_bits_dual
#endif
#ifdef VARIABLE_SPINDLE
    byte {uint8_t} is_pwm_rate_adjusted ' Tracks motions that require constant laser power/rate
#endif
'    st_block_t
...
VAR
    ...
    long segment_t  'typedef struct 
    word {uint16_t} n_step           ' Number of step events to be executed for this segment
    word {uint16_t} cycles_per_tick  ' Step distance traveled per ISR tick, aka step rate.
    byte {uint8_t} st_block_index   ' Stepper block data index. Uses this information to execute this segment.
#ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
    byte {uint8_t} amass_level    ' Indicates AMASS level for the ISR to execute this segment
#else
    byte {uint8_t} prescaler      ' Without AMASS, a prescaler is required to adjust for slow timing.
#endif
#ifdef VARIABLE_SPINDLE
    byte {uint8_t} spindle_pwm
#endif
'    segment_t
...
VAR
    ...
    long stepper_t  'typedef struct 
  ' Used by the bresenham line algorithm
    long {uint32_t} counter_x        ' Counter variables for the bresenham line tracer
    long counter_y
    long counter_z
#ifdef STEP_PULSE_DELAY
    byte {uint8_t} step_bits  ' Stores out_bits output to complete the step pulse delay
#endif

    byte {uint8_t} execute_step     ' Flags step execution for each interrupt.
    byte {uint8_t} step_pulse_time  ' Step pulse reset time after step rise
    byte {uint8_t} step_outbits         ' The next stepping-bits to be output
    byte {uint8_t} dir_outbits
#ifdef ENABLE_DUAL_AXIS
    byte {uint8_t} step_outbits_dual
    byte {uint8_t} dir_outbits_dual
#endif
#ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
    long {uint32_t} steps[N_AXIS]
#endif

    word {uint16_t} step_count       ' Steps remaining in line segment motion
    byte {uint8_t} exec_block_index ' Tracks the current st_block index. Change indicates new block.
...
VAR
    ...
    byte {static volatile uint8_t} segment_buffer_tail
    byte {static uint8_t} segment_buffer_head
    byte {static uint8_t} segment_next_head

    ' Step and direction port invert masks.
    byte {static uint8_t} step_port_invert_mask
    byte {static uint8_t} dir_port_invert_mask
#ifdef ENABLE_DUAL_AXIS
    byte {static uint8_t} step_port_invert_mask_dual
    byte {static uint8_t} dir_port_invert_mask_dual
#endif

    ' Used to a ISR nesting of the "Stepper Driver Interrupt". Should never occur though.
    byte {static volatile uint8_t} busy

    ' Pointers for the step segment being prepped from the planner buffer. Accessed only by the
    ' main program. Pointers may be planning segments or planner blocks ahead of what being executed.
    long {static plan_block_t *} pl_block     ' Pointer to the planner block being prepped
    long {static st_block_t *} st_prep_block  ' Pointer to the stepper block data being prepped

    ' Segment preparation data struct. Contains all the necessary information to compute new segments
    ' based on the current executing planner block.
VAR

    long st_prep_t  'typedef struct 
    byte {uint8_t} st_block_index  ' Index of stepper common data block being prepped
    byte {uint8_t} recalculate_flag

    long {float} dt_remainder
    long {float} steps_remaining
    long {float} step_per_mm
    long {float} req_mm_increment

#ifdef PARKING_ENABLE
    byte {uint8_t} last_st_block_index
    long {float} last_steps_remaining
    long {float} last_step_per_mm
    long {float} last_dt_remainder
#endif

    byte {uint8_t} ramp_type      ' Current segment ramp state
    long {float} mm_complete      ' End of velocity profile from end of current planner block in (mm).
                          ' NOTE: This value must coincide with a step(no mantissa) when converted.
    long {float} current_speed    ' Current speed at the end of the segment buffer (mm/min)
    long {float} maximum_speed    ' Maximum speed of executing block. Not always nominal speed. (mm/min)
    long {float} exit_speed       ' Exit speed of executing block (mm/min)
    long {float} accelerate_until ' Acceleration ramp end measured from end of block (mm)
    long {float} decelerate_after ' Deceleration ramp start measured from end of block (mm)

#ifdef VARIABLE_SPINDLE
    long {float} inv_rate    ' Used by PWM laser mode to speed up segment calculations.
    byte {uint8_t} current_spindle_pwm 
#endif
'    st_prep_t
...
PUB st_go_idle | {bool} pin_state
    ...
    if (((settings.stepper_idle_lock_time <> $ff) OR sys_rt_exec_alarm OR sys.state == STATE_SLEEP) AND sys.state <> STATE_HOMING) 
...
{{ "The Stepper Driver Interrupt" - This timer interrupt is the workhorse of Grbl. Grbl employs
   ...
   the Bresenham step event count, effectively multiplying it by 2, repeat while the axis step counts
   remain the same, and then double the stepper ISR frequency. In effect, we are allowing the
   non-dominant Bresenham axes step in the intermediate ISR tick, repeat while the dominant axis is
  ...
  ' Then pulse the stepping pins
    ...
    if (st.exec_segment == NULL) 
        ...
        if (segment_buffer_head <> segment_buffer_tail) 
            ...
            TCCR1B := (TCCR1B & !($07<<CS10)) | (st.exec_segment->prescaler<<CS10)
#endif
...
PUB st_generate_step_dir_invert_masks | {uint8_t} idx
...
PUB st_reset
    ...
    bytefill(@prep, 0, {sizeof}st_prep_t)
    bytefill(@st, 0, {sizeof}stepper_t)
    st.exec_segment := NULL
...
PUB st_update_plan_block_parameters
...
PUB {static uint8_t} st_next_block_index({uint8_t} block_index)
...
PUB st_prep_buffer | {uint8_t}idx, {float pl_block->step_event_count}, {float} inv_2_accel, {float} decel_dist, {float} exit_speed_sqr, nominal_speed, nominal_speed_sqr, intersect_distance, {segment_t *} prep_segment, {float} dt_max, dt, time_var, mm_var, speed_var, mm_remaining, minimum_mm, rpm, step_dist_remaining, n_steps_remaining, last_n_steps_remaining, inv_rate, {uint32_t} cycles
'YYY
    ' Block step prep buffer, repeat while in a suspend state and there is no suspend motion to execute.
    if (bit_istrue(sys.step_control, STEP_CONTROL_END_MOTION)) 
        return 
    repeat while (segment_buffer_tail <> segment_next_head)
        ...
        ' Query planner for a queued block
          ...
          ' Check if we need to only recompute the velocity profile or load a new block.
            ...
            ' Initialize segment buffer data for generating the segments.
                prep.steps_remaining := {float}pl_block->step_event_count 'XXX
                prep.step_per_mm := prep.steps_remaining/pl_block->millimeters
                prep.req_mm_increment := REQ_MM_INCREMENT_SCALAR/prep.step_per_mm
                prep.dt_remainder := 0.0 ' Reset for new segment block

                if ((sys.step_control & STEP_CONTROL_EXECUTE_HOLD) OR (prep.recalculate_flag & PREP_FLAG_DECEL_OVERRIDE))
    		...
    		if (sys.step_control & STEP_CONTROL_EXECUTE_HOLD)
    		    ...
    		    intersect_distance := 0.5*(pl_block->millimeters+inv_2_accel*(pl_block->entry_speed_sqr-exit_speed_sqr))
...
'indent level 2
        ...
        prep_segment->st_block_index := prep.st_block_index  'XXX
        ...
        'mm_var ' mm-Distance worker variable
        'speed_var ' Speed worker variable
        mm_remaining := pl_block->millimeters ' New segment distance from end of block. 'XXX
        minimum_mm := mm_remaining-prep.req_mm_increment ' Guarantee at least one step. 'XXX
        if (minimum_mm < 0.0)
            minimum_mm := 0.0
        repeat
            case (prep.ramp_type) 
                ...
                RAMP_ACCEL:
                    ' NOTE: Acceleration ramp only computes during first do-repeat while loop.
                    speed_var := pl_block->acceleration*time_var
                    mm_remaining -= time_var*(prep.current_speed + 0.5*speed_var)
            ...
            OTHER: ' RAMP_DECEL:
                ...
                if (prep.current_speed > speed_var) ' Check if at or below zero speed.
                    ...
                    if (mm_var > prep.mm_complete) ' Typical case. In deceleration ramp.
                        mm_remaining := mm_var
                        prep.current_speed -= speed_var
                        ' Segment complete. Exit case-statement. Continue do-repeat while loop.
        ...
        if (st_prep_block->is_pwm_rate_adjusted OR (sys.step_control & STEP_CONTROL_UPDATE_SPINDLE_PWM)) 
        ...
        step_dist_remaining := prep.step_per_mm*mm_remaining ' Convert mm_remaining to steps 'XXX
        ...
        prep_segment->n_step := last_n_steps_remaining-n_steps_remaining ' Compute number of steps to execute. 'XX

        ' Bail if we are at the end of a feed hold and don't have a step to execute.
        if (prep_segment->n_step == 0)   'XXX
            if (sys.step_control & STEP_CONTROL_EXECUTE_HOLD)
                ' Less than one step to decelerate to zero speed, but already very close. AMASS
                ' requires full steps to execute. So, just bail.
                bit_true(sys.step_control, STEP_CONTROL_END_MOTION)
#ifdef PARKING_ENABLE
                if (NOT(prep.recalculate_flag & PREP_FLAG_PARKING))
        ...
        if (cycles < (1{UL} << 16))
            ...
            prep_segment->cycles_per_tick:= $ffff  ' Just set the slowest speed possible.
#else
        ...
        elseif (cycles < (1UL << 19))
            ...
            ' < 4194304 (262ms@16MHz)
                ...
                prep_segment->cycles_per_tick := $ffff
#endif
...
PUB{float} st_get_realtime_rate
