...
PUB {static} report_util_line_feed
    printPgmString(PSTR("\r\n"))   'string(10, 13)?

PUB {static} report_util_feedback_line_feed
    serial_write("]")
    report_util_line_feed

PUB {static} report_util_gcode_modes_G
    printPgmString(PSTR(" G")) 'string(" G")?

PUB {static} report_util_gcode_modes_M
...
{{
static  report_util_setting_string(byte n) 
  ...
  case(n) 
    ...
    OTHER:
      ...
      byte idx:= 0
      repeat while (n => AXIS_SETTINGS_INCREMENT) 
...
PUB report_alarm_message({uint8_t} alarm_code)
    ...
    report_util_line_feed
    delay_ms(500)   'time.MSleep(500)   ' Force delay to ensure message clears serial write buffer.
...
' Welcome message
PUB report_init_message

    printPgmString(PSTR("\r\nGrbl " GRBL_VERSION " [ for help]\r\n"))
            'string(10, 13, "Grbl ", GRBL_VERSION, " [ for help]", 10, 13")

' Grbl help message
PUB report_grbl_help 
...
PUB report_grbl_settings | {uint8_t} idx, set_idx, val
    ...
    repeat set_idx from 0 to AXIS_N_SETTINGS-1
        repeat idx from 0 to N_AXIS-1
            case (set_idx) 
                0: report_util_float_setting(val+idx,settings.steps_per_mm[idx],N_DECIMAL_SETTINGVALUE)
                1: report_util_float_setting(val+idx,settings.max_rate[idx],N_DECIMAL_SETTINGVALUE)
                2: report_util_float_setting(val+idx,settings.acceleration[idx]/(60*60),N_DECIMAL_SETTINGVALUE)
                3: report_util_float_setting(val+idx,-settings.max_travel[idx],N_DECIMAL_SETTINGVALUE)
        val += AXIS_SETTINGS_INCREMENT

' Prints current probe parameters. Upon a probe command, these parameters are updated upon a
' successful probe or upon a failed probe with the G38.3 without errors command (if supported).
' These values are retained until Grbl is power-cycled, whereby they will be re-zeroed.
PUB report_probe_parameters | {float} print_position[N_AXIS]
...
PUB report_ngc_parameters | {float} coord_data[N_AXIS], {uint8_t} coord_select
    ...
    repeat coord_select from 0 to SETTING_INDEX_NCOORD
        ...
        case (coord_select) 
            6: printPgmString(PSTR("28"))
            7: printPgmString(PSTR("30"))
            OTHER: print_uint8_base10(coord_select+54)  ' G54-G59
        serial_write(":")
...
PUB report_gcode_modes
    ...
    report_util_gcode_modes_M
    case (gc_state.modal.spindle) 
        SPINDLE_ENABLE_CW : serial_write("3")
        SPINDLE_ENABLE_CCW : serial_write("4")
        SPINDLE_DISABLE : serial_write("5")
...
PUB report_build_info({char *}line)
...
PUB report_echo_line_received({char *}line)
...
PUB report_realtime_status | {uint8_t}idx, {int32_t}current_position[N_AXIS], {float}print_position, {float}wco[N_AXIS], {uint32_t} ln, {uint8_t} lim_pin_state, ctrl_pin_state, prb_pin_state, sp_state, cl_state
  ...
  ' Report current machine state and sub-states
    ...
    case (sys.state)
        ...
        STATE_JOG: printPgmString(PSTR("Jog"))
        STATE_HOMING: printPgmString(PSTR("Home"))
        STATE_ALARM: printPgmString(PSTR("Alarm"))
        STATE_CHECK_MODE: printPgmString(PSTR("Check"))
        STATE_SAFETY_DOOR:
    ...
    lim_pin_state := limits_get_state
    ctrl_pin_state := system_control_get_state
    ...
    if (sys.report_ovr_counter > 0)
        ...
        sp_state := spindle_get_state
        cl_state := coolant_get_state
        if (sp_state OR cl_state)
...
#ifdef DEBUG
PUB{void} report_realtime_debug
