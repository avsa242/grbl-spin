...
PUB mc_line({float *}target, {plan_line_data_t *}pl_data)
...
PUB mc_arc({float *}target, {plan_line_data_t *}pl_data, {float *}position, {float *}offset, {float }radius, {byte }axis_0, {uint8_t }axis_1, {uint8_t }axis_linear, {uint8_t }is_clockwise_arc) | {float}center_axis0, center_axis1, r_axis0, r_axis1, rt_axis0, rt_axis1, angular_travel, {uint16_t}segments, {float}theta_per_segment, linear_per_segment, cos_T, sin_T, cos_Ti, sin_Ti, r_axisi, {uint16_t}i, {uint8_t}count
    ...
    {{ Vector rotation by transformation matrix: r is the original vector, r_T is the rotated vector,
       ...
       cases. So, exact arc path correction is implemented. This approach as the problem of too many very
       ...
       low of !4 to a high of ~20 or so to a trig operations repeat while keeping arc generation accurate.
...
#endif
    ' Search to engage all axes limit casees at faster homing seek rate.
...
PUB{uint8_t} mc_probe_cycle({float *}target, {plan_line_data_t *}pl_data, {byte }parser_flags) | {uint8_t}is_probe_away, is_no_error, plan_status
    ...
    is_probe_away := bit_istrue(parser_flags, GC_PARSER_PROBE_IS_AWAY)
    is_no_error := bit_istrue(parser_flags, GC_PARSER_PROBE_IS_NO_ERROR)
...
PUB mc_override_ctrl_update({byte }override_state)
...
' active processes in the system. This also checks if a system reset is issued repeat while Grbl
