...
PUB protocol_main_loop | {byte} line_flags, char_counter, c
...
PUB protocol_exec_rt_system | {byte} rt_exec, {uint8_t} new_f_override, new_r_override, last_s_override, coolant_state
    ...
    if (rt_exec)
        ...
        if (rt_exec & EXEC_CYCLE_START) 
            ...
            if (!(rt_exec & (EXEC_FEED_HOLD | EXEC_MOTION_CANCEL | EXEC_SAFETY_DOOR))) 
                ...
                if ((sys.state == STATE_SAFETY_DOOR) AND !(sys.suspend & SUSPEND_SAFETY_DOOR_AJAR)) 
        ...
        if (rt_exec & EXEC_CYCLE_STOP)
            ...
            if ((sys.state & (STATE_HOLD|STATE_SAFETY_DOOR|STATE_SLEEP)) AND !(sys.soft_limit) AND !(sys.suspend & SUSPEND_JOG_CANCEL)) 
...
' template
PUB {static void} protocol_exec_rt_suspend | {float} restore_target[N_AXIS], parking_target[N_AXIS], retract_waypoint, {plan_block_t *} block, {byte} restore_condition
